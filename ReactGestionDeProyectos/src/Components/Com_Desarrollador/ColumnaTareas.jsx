...
return (
    <div className='flex flex-col gap-2 w-full'>
        <button onClick={()=> setOpen(!open)} 
            className='w-full bg-blueDashboard rounded-xl p-2 md:p-4 text-white 
        ...
        <div>
            <Droppable droppableId={estadoID}>
                {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className={`flex flex-col gap-3 transition-all duration-300 ${open ? 'max-h-500 opacity-100 mt-2' : 'max-h-0 lg:max-h-full opacity-0'}`}>
                            {
